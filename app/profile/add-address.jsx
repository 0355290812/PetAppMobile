...
const AddAddress = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert('Lỗi', 'Không thể thêm địa chỉ mới. Vui lòng thử lại sau.');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      >
        <View className="bg-white p-4 rounded-xl shadow-sm mb-4">
          ...
          <View className="mt-2 mb-2">
            <Checkbox value={isDefault.toString()} onChange={setIsDefault} size="md" className=''>
              <CheckboxIndicator mr="$2" className=''>
                <CheckboxIcon as={Check} className='' />
              </CheckboxIndicator>
              <CheckboxLabel>Đặt làm địa chỉ mặc định</CheckboxLabel>
            </Checkbox>
          </View>
        </View>
