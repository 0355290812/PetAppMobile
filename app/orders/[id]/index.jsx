...
import {Package, Truck, CreditCard, MapPin, FileText, CircleAlert, Clock, ChevronLeft, Star} from 'lucide-react-native';
...
const statusMap = {
    pending: {text: 'Đang xử lý', color: 'orange', icon: Clock},
    checkout: {text: 'Chờ thanh toán', color: 'indigo', icon: Package},
    shipping: {text: 'Đang giao hàng', color: 'purple', icon: Truck},
    delivered: {text: 'Hoàn thành', color: 'green', icon: Package},
...
const OrderDetailScreen = () => {
    ...
    // Add this function to navigate to the rating page
    const handleRateProduct = (productId) => {
        router.push({
            pathname: `/orders/${ id }/rate`,
            params: {productId}
        });
    };
                            ...
                            {order.items.map((item) => (
                                <HStack key={item._id} className="space-x-3">
                                    <Image
                                        ...
                                        className="w-20 h-20 rounded-md bg-gray-100 mr-2"
                                        resizeMode="cover"
                                    />
                                    <VStack className="flex-1 justify-between py-1">
                                        ...
                                        {order.status === 'completed' && (
                                            <Button
                                                className="mt-2 bg-amber-500 py-1 px-3 rounded-lg self-end"
                                                onPress={() => handleRateProduct(item._id)}
                                            >
                                                <HStack className="items-center space-x-1">
                                                    <Icon as={Star} size="xs" color="white" />
                                                    <ButtonText className="text-xs">Đánh giá</ButtonText>
                                                </HStack>
                                            </Button>
                                        )}
                                    </VStack>
